...
from oha.legacy_migration import (
...
from oha.runtime_convergence import (
    canonical_sync_coverage_status,
    confirmed_profile_input_status,
    legacy_writer_status,
    legacy_workbook_freshness_status,
    latest_migration_audit,
    migration_audit_integrity_status,
    pending_migration_status,
    require_scheduler_install_convergence,
    workbook_convergence_status,
)
...
from oha.web_view import create_dashboard_server
...
def _public_exception_payload(exc: Exception) -> dict[str, str]:
    ...
    elif isinstance(exc, LegacyMigrationConflictError):
        code = "legacy_migration_conflict"
        message = (
            "The legacy migration has unresolved record conflicts; review a fresh dry-run "
            "before retrying."
        )
    ...
    with lock_for(config):
        ...
        with open_existing_database(config) as database:
            try:
                ...
                migration_audit = latest_migration_audit(database)
                database.begin_sync(
                    batch_id,
                    start,
                    end,
                    ...
                    migration_id=(
                        migration_audit.get("migration_id")
                        if isinstance(migration_audit, dict)
                        and migration_audit.get("_invalid") is not True
                        else None
                    ),
                )
            except Exception as exc:
...
def command_migrate_legacy_workbook(arguments: argparse.Namespace) -> dict[str, Any]:
    config = open_config(arguments)
    source = Path(arguments.source)
    if arguments.dry_run:
        return preview_legacy_workbook(config, source)
    if not arguments.backup_source:
        raise ValueError("a verified source backup is required for a real legacy migration")
    _require_private_home_storage_consent(config)
    if not _cloud_workbook_export_allowed(config):
        raise ValueError(
            "cloud-workbook consent is required before a legacy migration can project health data"
        )
    ...
    with lock_for(config):
        ...
        with open_existing_database(config) as database:
            return migrate_legacy_workbook(
                config,
                database,
                source,
                backup_source=True,
...
def command_doctor(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    # A scheduler definition belonging to another temporary/test home must not
    # turn optional ghealth support into a required dependency here. A runtime
    # with no definition is different: it is an orphan that can still execute
    # and therefore keeps ghealth a required dependency until it is stopped.
    definition_exists = bool(schedule.get("definition_exists"))
    orphan_runtime = bool(
        schedule.get("runtime_orphaned")
        or (
            not definition_exists
            and any(
                schedule.get(key)
                for key in ("loaded", "active", "enabled")
            )
        )
    )
    ghealth_required = bool(schedule.get("installed") or orphan_runtime)
    for label, path in (
    ...
    checks.append(legacy_writer_status(Path.home()))
    ...
    checks.append(confirmed_profile_input_status(config.home_path / "profile.json"))
    ...
    if config.database.exists():
        try:
            with HealthDatabase(config.database, create=False, read_only=True) as database:
                ...
                pending_migration = pending_migration_status(database)
                migration_audit_integrity = migration_audit_integrity_status(database)
                workbook_convergence = workbook_convergence_status(config, database)
                legacy_freshness = legacy_workbook_freshness_status(database)
                sync_coverage = canonical_sync_coverage_status(
                    config,
                    database,
                    Path(arguments.legacy_workbook)
                    if getattr(arguments, "legacy_workbook", None)
                    else None,
                )
            checks.append({"check": "SQLite integrity", "ok": integrity == "ok", "detail": integrity})
            for convergence_check in (
                pending_migration,
                migration_audit_integrity,
                workbook_convergence,
                legacy_freshness,
                sync_coverage,
            ):
    ...
    with lock_for(config):
        ...
        with open_existing_database(config) as database:
            require_scheduler_install_convergence(
                config,
                database,
                legacy_writer_status(Path.home()),
            )
...
def command_web(arguments: argparse.Namespace) -> dict[str, Any]:
    config = open_config(arguments)
    with HealthDatabase(config.database, create=False, read_only=True):
        pass
    server = create_dashboard_server(
        config.database, host="127.0.0.1", port=arguments.port
    )
    port = int(server.server_address[1])
    url = f"http://127.0.0.1:{port}/"
    print(
        json.dumps(
            {"status": "serving", "url": url, "read_only": True},
            ensure_ascii=True,
        ),
        flush=True,
    )
    ...
    try:
        ...
        server.server_close()
    return {"status": "stopped", "url": url, "read_only": True}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    doctor.add_argument(
        "--legacy-workbook",
        help="Read-only legacy workbook to include in convergence checks",
    )
    ...
    migrate = sub.add_parser("migrate", help="Run an explicit, controlled data migration")
    migrate_sub = migrate.add_subparsers(dest="migration_kind", required=True)
    legacy = migrate_sub.add_parser(
        "legacy-workbook",
        help="Preview or import structured health facts from a legacy workbook",
    )
    legacy.add_argument("--source", required=True)
    legacy.add_argument("--dry-run", action="store_true")
    legacy.add_argument("--backup-source", action="store_true")
    legacy.set_defaults(handler=command_migrate_legacy_workbook)
    ...
    web = sub.add_parser(
        "web", help="Open a localhost-only, read-only HTML browser for raw health data"
    )
