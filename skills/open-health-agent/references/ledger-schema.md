...
- [Controlled legacy-workbook migration](#controlled-legacy-workbook-migration)
...
## Controlled legacy-workbook migration

`migrate legacy-workbook` is an explicit one-time bridge from a supported old
Excel health ledger into SQLite. The source remains read-only; unknown and
non-health sheets are counted but their cells are not imported. Always run the
read-only preview first and stop on conflicts:

```bash
open-health-agent migrate legacy-workbook --source '/private/legacy.xlsx' --dry-run
open-health-agent migrate legacy-workbook --source '/private/legacy.xlsx' --backup-source
```

The real run requires a verified full-source backup, the common writer lock,
one batch transaction, migration witnesses, atomic workbook projection, and
postcondition checks. A failure after the SQLite commit remains explicitly
pending and may be resumed only from the unchanged source; it must never be
reported as a completed migration. See `docs/migration.md` for cutover and
rollback rules.
